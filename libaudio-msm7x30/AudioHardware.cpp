...
void addToTable(int decoder_id, int device_id, int device_id_tx, int stream_type, bool active) {
    ALOGD("addToTable (dec_id %d, dev_rx %d, dev_tx %d, type %d, active %d)", decoder_id, device_id, device_id_tx, stream_type, active);
...
void deleteFromTable(int Stream_type) {
    ALOGD("deleteFromTable, type %d", Stream_type);
...
int enableDevice(int device, short enable) {
    ALOGD("value of device and enable is %d %d", device, enable);
    ...
    mEffectEnabled(false)
{
    ...
    for (i = 0; i < dev_cnt; i++) {
        ...
        if (device_list[index].dev_id >= 0) {
            ALOGI("Found device: %s:index = %d, dev_id: %d", ( char* )name[i],
        ...
        ALOGI("class ID = %d,capablity = %d for device %d", device_list[index].class_id,
...
AudioStreamOut* AudioHardware::openOutputStream(uint32_t devices, int *format,
    ...
    { /* scope for the lock */
        ...
        ALOGD("AudioHardware::openOutputStream");
...
status_t AudioHardware::setMicMute_nosync(bool state) {
    if (mMicMute != state) {
        ...
        if (isStreamOnAndActive(PCM_REC) && (mMode == android_audio_legacy::AudioSystem::MODE_IN_COMMUNICATION)) {
            vMicMute = state;
            ALOGD("VOIP Active: vMicMute %d", vMicMute);
...
static status_t set_volume_rpc(uint32_t device,
                               uint32_t method,
                               uint32_t volume) {
    ALOGV("set_volume_rpc(%d, %d, %d)", device, method, volume);
...
status_t AudioHardware::setVoiceVolume(float v) {
    ...
    if (mCurSndDevice == SND_DEVICE_HAC) {
        ALOGI("HAC enable: Setting in-call volume to maximum.");
        if (msm_set_voice_rx_vol(VOICE_VOLUME_MAX)) {
            ALOGE("msm_set_voice_rx_vol(%d) failed errno = %d", VOICE_VOLUME_MAX, errno);
            return -1;
        }
    } else {
        ALOGD("setVoiceVolume(%f)", v);
        ALOGI("Setting in-call volume to %d (available range is 0 to %d)", vol, VOICE_VOLUME_MAX);
...
status_t AudioHardware::doRouting(AudioStreamInMSM72xx *input) {
    ...
    if (input != NULL) {
        ...
        ALOGI("do input routing device 0x%x", inputDevice);
...
*/
            ...
            if (outputDevices & android_audio_legacy::AudioSystem::DEVICE_OUT_SPEAKER) {
                ALOGI("Routing audio to No microphone Wired Headset and Speaker (%d,%d)", mMode, outputDevices);
                sndDevice = SND_DEVICE_HEADPHONE_AND_SPEAKER;
            } else {
#ifdef HAVE_FM_RADIO
                if (outputDevices & android_audio_legacy::AudioSystem::DEVICE_OUT_FM) {
                    ALOGI("Routing FM audio to No microphone Wired Headset (%d,%d)", mMode, outputDevices);
                    sndDevice = SND_DEVICE_FM_HEADSET;
                } else {
                    ALOGI("Routing audio to No microphone Wired Headset (%d,%d)", mMode, outputDevices);
                    sndDevice = SND_DEVICE_NO_MIC_HEADSET;
                }
#else
                ALOGI("Routing audio to No microphone Wired Headset (%d,%d)", mMode, outputDevices);
        ...
        if (mMode == android_audio_legacy::AudioSystem::MODE_IN_CALL) {
            if (mHACSetting && hac_enable && mCurSndDevice == SND_DEVICE_HAC) {
                ...
                if (msm_set_voice_rx_vol(mVoiceVolume))
                    ALOGE("msm_set_voice_rx_vol(%d) failed errno = %d", mVoiceVolume, errno);
            }
        }
    }
...
status_t AudioHardware::enableFM(uint32_t sndDevice) {
    ...
    if (msm_route_stream(PCM_PLAY, 0, DEV_ID(cur_rx), 1))
        ALOGE("msm_route_stream(PCM_PLAY, 0, %d, 1) failed", DEV_ID(cur_rx));
...
status_t AudioHardware::disableFM() {
    ...
    ALOGD("Deroute fm playback stream");
...
status_t AudioHardware::AudioSessionOutMSM7xxx::setParameters(const String8& keyValuePairs) {
    ...
    if (param.getInt(key, device) == NO_ERROR) {
        mDevices = device;
        ALOGV("set output routing 0x%x", mDevices);
...
String8 AudioHardware::AudioSessionOutMSM7xxx::getParameters(const String8& keys) {
    ...
    if (param.get(key, value) == NO_ERROR) {
        ALOGV("get routing 0x%x", mDevices);
...
status_t AudioHardware::AudioSessionOutMSM7xxx::setVolume(float left, float right) {
    ...
    if (v < 0.0) {
        ALOGW("AudioSessionOutMSM7xxx::setVolume(%f) under 0.0, assuming 0.0", v);
        v = 0.0;
    } else if (v > 1.0) {
        ALOGW("AudioSessionOutMSM7xxx::setVolume(%f) over 1.0, assuming 1.0", v);
        v = 1.0;
    }

    /* Ensure to convert the log volume back to linear for LPA */
    float vol = v * 100;
    ALOGV("AudioSessionOutMSM7xxx::setVolume(%f)", v);
    ALOGV("Setting session volume to %f (available range is 0 to 100)", vol);
...
status_t AudioHardware::AudioStreamInMSM72xx::setParameters(const String8& keyValuePairs) {
    ...
    if (param.getInt(key, device) == NO_ERROR) {
        ALOGV("set input routing 0x%x", device);
        if (device & (device - 1))
            status = BAD_VALUE;
        else {
            mDevices = device;
